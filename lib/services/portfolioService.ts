...
export async function updatePortfolioProject(id: string, input: PortfolioProjectInput) {
  const project = await prisma.portfolioProject.update({ where: { id }, data: input });
  revalidateTag(PORTFOLIO_PUBLIC_TAG);
  return project;
}
...
export async function deletePortfolioProject(id: string) {
  ...
  await Promise.all(project.images.map((img) => deleteBlobImage(img.publicUrl)));
  await prisma.portfolioProject.delete({ where: { id } });
  revalidateTag(PORTFOLIO_PUBLIC_TAG);
}

export async function movePortfolioProject(id: string, direction: "up" | "down") {
  const all = await prisma.portfolioProject.findMany({ orderBy: { displayOrder: "asc" } });
...
export async function movePortfolioImage(imageId: string, direction: "up" | "down") {
  const image = await prisma.portfolioImage.findUniqueOrThrow({ where: { id: imageId } });
...
export async function reorderPortfolioProjects(orderedIds: string[]) {
  await prisma.$transaction(
    orderedIds.map((id, index) =>
      prisma.portfolioProject.update({ where: { id }, data: { displayOrder: index } })
    )
  );
  revalidateTag(PORTFOLIO_PUBLIC_TAG);
}
...
export async function removePortfolioImage(imageId: string) {
  ...
  if (image.isMain) {
    ...
    if (next) {
      await prisma.portfolioImage.update({ where: { id: next.id }, data: { isMain: true } });
    }
  }
  revalidateTag(PORTFOLIO_PUBLIC_TAG);
}

export async function reorderPortfolioImages(portfolioProjectId: string, orderedIds: string[]) {
  await prisma.$transaction(
    orderedIds.map((id, index) =>
      prisma.portfolioImage.update({ where: { id }, data: { displayOrder: index } })
    )
  );
  revalidateTag(PORTFOLIO_PUBLIC_TAG);
}

export async function setMainPortfolioImage(portfolioProjectId: string, imageId: string) {
  await prisma.$transaction([
    prisma.portfolioImage.updateMany({
      where: { portfolioProjectId },
      data: { isMain: false },
    }),
    prisma.portfolioImage.update({ where: { id: imageId }, data: { isMain: true } }),
  ]);
  revalidateTag(PORTFOLIO_PUBLIC_TAG);
}
