  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <LinkButtonSmall href="/admin/portfolio/new">
            Add project
          </LinkButtonSmall>
        }
      />
          ...
          {projects.map((project, index) => {
            ...
            return (
              ...
              >
                ...
                <Badge
                  label={project.published ? "Published" : "Hidden"}
                  tone={project.published ? "ACTIVE" : "DISABLED"}
                />
                {project.featured && <Badge label="Featured" tone="ACTIVE" />}

                <div className="flex items-center gap-1">
                  <form
                    action={movePortfolioProjectAction.bind(
                      null,
                      project.id,
                      "up",
                    )}
                  >
                    <button
                      type="submit"
                      aria-label={`Move ${project.titleEn} up`}
                      disabled={index === 0}
                      className="border border-rule px-2 py-1 text-xs disabled:opacity-30"
                    >
                      ↑
                    </button>
                  </form>
                  ...
                  >
                    <button
                      type="submit"
                      aria-label={`Move ${project.titleEn} down`}
                ...
                <form
                  action={setPortfolioFeaturedAction.bind(
                    null,
                    project.id,
                    !project.featured,
                  )}
                ...
                <form
                  action={setPortfolioPublishedAction.bind(
                    null,
                    project.id,
                    !project.published,
                  )}
