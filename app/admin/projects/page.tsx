...
import { PROJECT_STATUSES, isValidProjectStatus } from "@/lib/validation/shared";
...
export default async function AdminProjectsPage({
  ...
  const validStatus = status && isValidProjectStatus(status) ? status : undefined;
  const projects = await listProjectsForAdmin({ search: q, status: validStatus });

  return (
    <div>
      <PageHeader title="Projects" description="All active and completed customer projects." />
              ...
              {projects.map((p) => (
                <tr key={p.id} className="border-b border-black/5 last:border-0 hover:bg-black/[0.02]">
                  ...
                  <td className="px-5 py-3 text-ink/70">
                    {p.customer.businessName || p.customer.name || p.customer.email}
                  </td>
                  <td className="px-5 py-3">
