      ...
      <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card>
          ...
          <div className="mt-4 space-y-3">
            {recentRequests.length === 0 && <p className="text-sm text-muted">No new requests.</p>}
            {recentRequests.map((r) => (
              <Link
                key={r.id}
                href={`/admin/requests/${r.id}`}
                className="block border border-black/5 p-3 hover:border-black/15"
              >
                <p className="font-semibold">{r.businessName || r.name || r.email}</p>
                <p className="mt-0.5 line-clamp-1 text-sm text-muted">{r.description}</p>
              </Link>
            ))}
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="mt-4 space-y-3">
            {activeProjectsList.length === 0 && <p className="text-sm text-muted">No active projects.</p>}
            {activeProjectsList.map((p) => (
              ...
              >
                ...
                <p className="mt-0.5 text-sm text-muted">{p.customer.businessName || p.customer.name || p.customer.email}</p>
              </Link>
            ))}
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="mt-4 space-y-3">
            {recentSupport.length === 0 && <p className="text-sm text-muted">No support requests yet.</p>}
            {recentSupport.map((s) => (
              <Link
                key={s.id}
              ...
              >
                ...
                <p className="mt-0.5 line-clamp-1 text-sm text-muted">{s.message}</p>
              </Link>
            ))}
          </div>
        </Card>
