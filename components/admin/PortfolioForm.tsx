...
export function PortfolioForm({
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <AdminTextarea
          label="What we built (English, optional)"
          name="builtEn"
          rows={3}
          defaultValue={project?.builtEn ?? ""}
        />
        <AdminTextarea
          label="What we built (Arabic, optional)"
          name="builtAr"
          rows={3}
          dir="rtl"
          defaultValue={project?.builtAr ?? ""}
        />
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <AdminTextarea
          label="Result (English, optional)"
          name="resultEn"
          rows={3}
          defaultValue={project?.resultEn ?? ""}
        />
        <AdminTextarea
          label="Result (Arabic, optional)"
          name="resultAr"
          rows={3}
          dir="rtl"
