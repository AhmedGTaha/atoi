    ...
    <section id="home" className="hero">
      <Container className="hero-grid">
        <div>
          ...
          <p className="mt-8 max-w-lg text-lg leading-relaxed text-ink/80">
          ...
          <div className="mt-12 flex flex-wrap gap-x-6 gap-y-2 border-t pt-5">
            {[1, 2, 3, 4].map((n) => (
              <span key={n} className="section-marker text-muted">
        ...
        <BlueprintPanel className="work-order">
          <div className="flex justify-between border-b p-4 section-marker">
            <span>{locale === "ar" ? "مسار العمل" : "Work order"}</span>
            <span className="text-muted">ATOI / 01</span>
          </div>
          ...
          {[1, 2, 3].map((n) => (
            <div className="work-order-row" key={n}>
              ...
              <span aria-hidden="true" className="text-blue-dark">
                ↗
              </span>
            </div>
          ))}
