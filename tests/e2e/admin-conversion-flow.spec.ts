...
test.describe("Admin login and request conversion", () => {
  test("full flow: submit request -> admin login -> convert -> publish update", async ({ page }) => {
    ...
    await modal.getByLabel(/What can we help with/).fill("We need a loyalty app for our coffee shop chain.");
    ...
    await expect(page.getByText("loyalty app for our coffee shop")).toBeVisible();
    ...
    await page.getByRole("button", { name: "Create Customer & Project" }).click();

    await expect(page).toHaveURL(/\/admin\/projects\/[a-f0-9-]+$/);
    await expect(page.getByRole("heading", { name: "Coffee Loyalty App" })).toBeVisible();
    ...
    await page.getByLabel("Project update").fill("Kickoff call scheduled for next week.");
    await page.getByRole("button", { name: "Publish update" }).click();
    await expect(page.getByText(/email failed to send/i)).toBeVisible();
    await expect(page.getByText("Kickoff call scheduled for next week.")).toBeVisible();
  });
});
